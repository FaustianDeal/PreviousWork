...
/**
 * @class TryThisInsteadController
 */
class TryThisInsteadController {
  /**
   * Constructs instances of the TryThisInsteadController
  ...
  constructor($log, $filter, $element) {
    ...
    if (DEBUG_LOGGING) {
      this.debug = $log.debug.bind($log, 'TryThisInsteadController');
      this.debug('ctor');
    } else {
...
//
const name = 'PAD.try-this-instead';
angular
  .module(name, [])
  .component('tryThisInstead', {
    // template: template,
    controller: TryThisInsteadController,
    require: {
      leaflet: '^leaflet',
    },
    bindings: {
